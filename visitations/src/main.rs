...
const DATABASE: &str = "database describing tourists visiti";
...
async fn create_schema(connection: Connection) -> Result<(),  HandleError> {
...
async fn create_data(connection: Connection) ->  Result<(), HandleError> {
    let databases = DatabaseManager::new(connection.clone());
    // insert data
    ...
    for first_name in geo_first_names {
        ...
        for last_name in geo_last_names {
            ...
            let _ = transaction.query().insert(
                &create_visited_relationship(start, end, first_name, last_name, "London", 3)
                    .as_str(),
            );
        }
    }
    ...
    for first_name in uk_first_names {
        ...
        for last_name in uk_last_names {
            ...
            let _ = transaction.query().insert(
                &create_visited_relationship(start, end, first_name, last_name, "Zugdidi", 3)
                    .as_str(),
            );
        }
    }
